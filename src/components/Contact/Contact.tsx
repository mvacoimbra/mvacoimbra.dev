...
export default function Contact() {
	...
	useEffect(() => {
		...
		if (parent && div && divTwo) {
			...
			const keyframes = `
        @keyframes floating {
          0% { transform: translate(${Math.random() * parentWidth}px, ${
						Math.random() * parentHeight
					}px) }
          25% { transform: translate(${Math.random() * parentWidth}px, ${
						Math.random() * parentHeight
					}px) }
          50% { transform: translate(${Math.random() * parentWidth}px, ${
						Math.random() * parentHeight
					}px) }
          100% { transform: translate(${Math.random() * parentWidth}px, ${
						Math.random() * parentHeight
					}px) }
        }
      `;
      ...
      @keyframes floating2 {
        0% {
          transform: translate(${Math.random() * parentWidth}px, ${
						Math.random() * parentHeight
					}px);
           scale: 0.5;
        }
					...
					<div className="col-span-2 gap-12 flex flex-col justify-center max-xl:items-center">
						...
						<ul className="flex gap-7">
							{links.map((item, index) => {
								const itemKey = `${item.link}-${index}`;
								return (
									<li key={itemKey} className="text-synth-green1 text-6xl">
